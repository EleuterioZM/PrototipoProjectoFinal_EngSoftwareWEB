...
public class AvaliacaoServlet extends HttpServlet {
        ...
        if (action != null) {
            switch (action) {
                ...
                case "confirmarExclusao":
                    confirmarExclusaoAvaliacao(request, response);
                    break;
                    case "visualizar":
   ...
   @Override
    ...
    private void editAvaliacao(HttpServletRequest request, HttpServletResponse response)
        ...
        if (idStr != null && !idStr.isEmpty()) {
            ...
            if (avaliacao != null) {
                ...
                String url = "/Estudantes/FRMEditAvaliacao.jsp?id=" + avaliacao.getId() + "&descricao=" + avaliacao.getDescricao() + "&peso=" + avaliacao.getPeso();
    ...
    private void deleteAvaliacao(HttpServletRequest request, HttpServletResponse response)
        ...
        if (idStr != null && !idStr.isEmpty()) {
            ...
            if (avaliacao != null) {
                ...
                request.getRequestDispatcher("/confirmarExclusaoAvaliacao.jsp").forward(request, response);
            } else {
                response.sendRedirect(request.getContextPath() + "/AvaliacaoServlet");
            }
        } else {
    ...
    private void confirmarExclusaoAvaliacao(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String idStr = request.getParameter("id");
        if (idStr != null && !idStr.isEmpty()) {
            int id = Integer.parseInt(idStr);
    ...
    private void listAvaliacoes(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("/Estudantes/FRMListarAvaliacoes.jsp").forward(request, response);
    }
 ...
 private void visualizarAvaliacao(HttpServletRequest request, HttpServletResponse response)
    ...
    if (idStr != null && !idStr.isEmpty()) {
        ...
        if (avaliacao != null) {
            ...
            String url = "/Estudantes/ViewAvalicao.jsp?id=" + avaliacao.getId() + "&descricao=" + avaliacao.getDescricao() + "&peso=" + avaliacao.getPeso();
            response.sendRedirect(request.getContextPath() + url);
        } else {
            response.sendRedirect(request.getContextPath() + "/AvaliacaoServlet");
        }
